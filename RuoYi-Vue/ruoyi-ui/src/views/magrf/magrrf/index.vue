...
<script>
import { getReviewedReferrals, getReviewedReferral, deleteReviewedReferral, addReferral,  updateReferral,searchReviewedReferral} from "@/api/magrf/magrrf";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getReviewedReferrals(this.queryParams).then(response => {
    ...
    handleQuery() {
      ...
      searchByTitle(){
        ...
        searchReviewedReferral(this.queryParams).then(response => {
          this.referralList = response.rows;
          this.total = response.total;
          this.loading = false;
      });
    ...
    /** 详情按钮操作 */
    handleDetail(row) {
      getReviewedReferral(row.referralID).then(response => {
    ...
    handleDelete(row) {
      this.$confirm('是否确认删除此内推信息?', '确认', {
        ...
        deleteReviewedReferral(row.referralID).then(response => {
          this.$message({
            type: 'success',
            message: '删除成功'
          });
  ...
  computed : {
    referralstatusText() {
      return this.form.referralstatus === 1 ? '待审核' : '已通过';
  }
}
};
</script>
